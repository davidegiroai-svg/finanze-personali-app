...
FIXED_KEYWORDS = {
    ...
    "Abbonamenti ricorrenti": ["netflix", "spotify", "prime video", "now tv", "disney", "abbonamento", "subscription", "telefono", "mobile", "internet", "fibra", "iliad", "hype plus", "canone"],
    "Rate & debiti": ["prestito", "loan", "finanziamento", "rate", "credito al consumo"],
}

VARIABLE_KEYWORDS = {
    "Cene & aperitivi": ["ristorante", "restaurant", "trattoria", "osteria", "pizzeria", "pub", "bar", "caffe", "caffè", "aperitivo", "apericena", "kebab"],
    "Spesa supermercato": ["esselunga", "coop", "iper", "ipercoop", "conad", "carrefour", "lidl", "md", "pam", "aldi", "eurospin", "eurospar", "bennet", "mercato"],
    "Trasporti & mobilità": ["atm", "trenitalia", "italo", "uber", "bolt", "taxi", "carburante", "benzina", "diesel", "telepass", "enjoy", "share now", "sharenow", "free now", "freenow", "tper", "ridemovi", "buffet"],
    "Sport & benessere": ["palestra", "gym", "fitness", "decathlon", "sport center", "yoga", "pilates", "spa", "wellness"],
    "Salute": ["farmacia", "pharmacy", "medico", "analisi", "ticket", "dentista", "ottico", "occhiali", "visita", "esame", "unobravo"],
    "Tabacco & vizi": ["tabacchi", "tabaccheria", "sigarette", "tobacco", "svapo", "vape", "sisal", "lotto", "scommessa", "palabingo"],
    "Shopping & extra": ["amazon", "zalando", "zara", "h&m", "hm", "mediaworld", "unieuro", "ikea", "temu"],
    "Cultura & formazione": ["libreria", "feltrinelli", "ibs", "corso", "master", "udemy", "coursera", "libro", "libri", "cinema"],
    "Casa & arredo": ["brico", "leroy merlin", "obi", "ikea", "casaforte", "arredo", "risparmio casa"],
    "Animali": ["zooplus", "arcaplanet", "pet shop", "toelettatura"],
    "Viaggi": ["booking.com", "airbnb", "hotel", "ryanair", "easyjet", "wizzair", "albergo"],
    "Regali": ["regalo", "gift", "fiori", "florist", "flower"],
}

SAVINGS_INVEST_KEYWORDS = {
    "Risparmio conto / deposito": ["conto deposito", "risparmio", "saving", "savings", "deposito", "caparra"],
    "Investimenti azioni/ETF": ["degiro", "etoro", "revolut trading", "trade republic", "fineco", "directa", "interactive brokers", "broker"],
    "Crypto & speculativi": ["binance", "coinbase", "kraken", "crypto.com", "bitpanda"],
    "Altri investimenti": ["polizza", "assicurazione vita", "gestione patrimoniale"],
}
...
def normalize_merchant(desc: str) -> str:
    ...
    remove_tokens = ["pagamento pos", "pagamento carta", "acquisto carta", "operazione pos", "contactless", "e-commerce", "ecommerce", "pagamento"]
...
def ai_batch_categorize(transactions: list, client: Groq) -> dict:
    if client is None or len(transactions) == 0:
        return {}
    try:
        trans_list = "\n".join([f"{i}. Descrizione: '{t['description']}', Importo: {t['amount']}€" for i, t in enumerate(transactions)])
...
Usa SOLO i nomi esatti delle sottocategorie della lista."""
        response = client.chat.completions.create(model=st.secrets["groq"]["model"], messages=[{"role": "user", "content": prompt}], temperature=0.1, max_tokens=2000)
...
Fornisci consigli pratici, con numeri specifici e percentuali. Sii conciso (max 4 punti)."""
        response = client.chat.completions.create(model=st.secrets["groq"]["model"], messages=[{"role": "user", "content": prompt}], temperature=0.7, max_tokens=500)
...
def build_internal_df(df_raw, col_date, col_desc, col_amount, col_name=None, col_type=None, col_iban=None):
    ...
    if col_name and col_name in df.columns:
        df["description"] = (df[col_name].astype(str).fillna("") + " - " + df[col_desc].astype(str).fillna("")).str.strip(" -")
    else:
        df["description"] = df[col_desc].astype(str)
    raw_amount = df[col_amount].astype(str).str.strip().str.replace("€", "", regex=False).str.replace("\u20ac", "", regex=False).str.replace(" ", "", regex=False)
    def parse_amount(x):
    ...
    if col_type and col_type in df.columns:
        ...
        df["bank_category"] = ""
    df["direction"] = df["amount"].apply(lambda x: "Entrata" if x is not None and x > 0 else "Uscita")
    df["macro_category"] = ""
    df["subcategory"] = ""
    df["normalized_merchant"] = ""
    df = df.sort_values("date")
    cols = ["date", "description", "amount", "account", "bank_category", "direction", "macro_category", "subcategory", "normalized_merchant"]
    return df[cols]
if uploaded_file is not None:
    ...
    with col1:
        col_date = st.selectbox("Colonna DATA operazione", options=columns, index=columns.index(default_date) if default_date in columns else 0)
        col_amount = st.selectbox("Colonna IMPORTO", options=columns, index=columns.index(default_amount) if default_amount in columns else 0)
    with col2:
        col_desc = st.selectbox("Colonna DESCRIZIONE", options=columns, index=columns.index(default_desc) if default_desc in columns else 0)
        col_name = st.selectbox("Colonna NOME / merchant (opzionale)", options=["(nessuna)"] + columns, index=(columns.index(default_name) + 1) if default_name in columns else 0)
    with col3:
        col_type = st.selectbox("Colonna TIPOLOGIA (opzionale)", options=["(nessuna)"] + columns, index=(columns.index(default_type) + 1) if default_type in columns else 0)
        col_iban = st.selectbox("Colonna IBAN / conto (opzionale)", options=["(nessuna)"] + columns, index=(columns.index(default_iban) + 1) if default_iban in columns else 0)
    
    if st.button("✅ Conferma mappatura e prepara dati"):
        df_internal = build_internal_df(df_raw, col_date, col_desc, col_amount, None if col_name == "(nessuna)" else col_name, None if col_type == "(nessuna)" else col_type, None if col_iban == "(nessuna)" else col_iban)
        ...
        if use_ai and groq_client:
            ...
            if len(uncategorized) > 0:
                ...
                trans_for_ai = [{"description": row["description"], "amount": row["amount"]} for _, row in uncategorized.iterrows()]
        ...
        st.download_button(label="💾 Scarica dati categorizzati (CSV)", data=csv_export, file_name=f"finanze_categorizzate_{datetime.now().strftime('%Y%m%d')}.csv", mime="text/csv")
        ...
        with col_c:
            ...
            periodo = f"{data_min.date()} → {data_max.date()}" if pd.notnull(data_min) and pd.notnull(data_max) else "N/D"
            st.metric("Periodo coperto", periodo)
        
        st.markdown("### 🎛 Filtro periodo")
        start_date, end_date = st.date_input("Seleziona intervallo date", value=(data_min.date() if pd.notnull(data_min) else date.today(), data_max.date() if pd.notnull(data_max) else date.today()))
        mask = df_categorized["date"].between(pd.to_datetime(start_date), pd.to_datetime(end_date))
        ...
        with col_c:
            spese_fisse = df_filtered.loc[df_filtered["macro_category"] == "Fisso", "amount_abs"].sum()
            spese_var = df_filtered.loc[df_filtered["macro_category"] == "Variabile", "amount_abs"].sum()
            st.metric("Spese fisse / variabili", f"Fisso: {spese_fisse:,.0f}€ • Var: {spese_var:,.0f}€")
        
        st.markdown("### 🥧 Macro-categorie (Entrate / Fissi / Variabili / Risparmi)")
        df_macro = df_filtered.copy()
        df_macro["value_for_chart"] = df_macro.apply(lambda r: r["amount"] if r["macro_category"] == "Entrata" else r["amount_abs"], axis=1)
        agg_macro = df_macro.groupby("macro_category")["value_for_chart"].sum().reset_index()
        if not agg_macro.empty:
            fig_macro = px.pie(agg_macro, names="macro_category", values="value_for_chart", hole=0.4, title="Distribuzione importi per macro-categoria")
        ...
        agg_sub = df_var.groupby("subcategory")["amount_abs"].sum().reset_index().sort_values("amount_abs")
        if not agg_sub.empty:
            fig_sub = px.bar(agg_sub, x="amount_abs", y="subcategory", orientation="h", title="Spese variabili per sottocategoria", labels={"amount_abs": "Importo €", "subcategory": "Categoria"})
        ...
        st.dataframe(df_filtered.drop(columns=["amount_abs"]), use_container_width=True, hide_index=True)

else:
    st.info("👆 Carica un CSV dal sidebar per iniziare.")
    st.markdown("""**Consigli CSV Hype**
